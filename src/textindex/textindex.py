...
class TextIndex:
    ...
    def inform(self, msg, severity="normal", force=False):
        should_echo = (
            force or self.verbose or severity == "warning" or severity == "error"
        )
        if should_echo:
            out = ""
            prefix = "TextIndex"
    ...
    def load_concordance_file(self, path):
        if not self.original_document or self.original_document == "":
            self.inform(
                f"No document to index; ignoring concordance file.", severity="warning"
            )
            return
        ...
        for term in term_ranges:
            ...
            conc_doc = (
                conc_doc[: term[0] + offset] + mark + conc_doc[term[1] + offset :]
            )
    ...
    def convert_latex_index_commands(self):
        if not self.original_document:
            self.inform(
                f"No original document text; can't convert latex commands.",
                severity="warning",
            )
            return
        ...
        for lmark in latex_matches:
            ...
            cmd_content = re.sub(
                r"(?i)\\(?:textbf|textit|textsl|emph)\{([^\}]+)\}", r"_\1_", cmd_content
            )
        ...
        self.inform(
            f"{marks_converted} latex index command{plural} converted to index mark{plural}.",
            force=True,
        )
        ...
        if self.original_document:
            ...
            for placeholder in idx_placeholders:
                if re.search(r"\bsection\b", placeholder.group(0)):
                    ...
                    self.inform(
                        f"Section mode enabled for all index directives in document.",
                        force=True,
                    )
                    break
            ...
            for directive in directive_matches:
                if self.section_mode:
                    ...
                    if headings:
                        ...
                        for head_match in headings:
                            ...
                            indexed_doc = (
                                indexed_doc[:start] + html_heading + indexed_doc[end:]
                            )
                ...
                if label_match:
                    ...
                    alias_definition_match = re.search(
                        TextIndex._alias_definition_pattern, label_path_components[-1]
                    )
                    ...
                    label_path_components = [
                        component.strip("'\"") for component in label_path_components
                    ]
                    ...
                    if alias_definition_match:
                        ...
                        if alias_definition_match.start() > 0:
                            ...
                            if len(label_path_components) == 0:
                                ...
                                if alias_name in self.aliases:
                                    ...
                                    label_path_components = self.aliases[alias_name][
                        ...
                        if alias_without_reference:
                            ...
                            span_html = (
                                f"{emph(span_contents) if span_contents else ''}"
                            )
                            indexed_doc = (
                                indexed_doc[: directive.start() + offset]
                                + span_html
                ...
                if suffix_match:
                    ...
                    params = (
                        params[: suffix_match.start()] + params[suffix_match.end() :]
                    )
                ...
                if cross_match:
                    ...
                    for ref in refs:
                        ...
                        ref_path_components = [
                            component.strip("'\"") for component in ref_path_components
                        ]
            ...
            indexed_doc = re.sub(
                TextIndex._index_placeholder_pattern, self.index_replace, indexed_doc
            )
    ...
    def define_alias(self, name, path):
        ...
        self.inform(
            f"\t{'Redefined existing' if redefinition else 'Defined new'} alias {TextIndex._alias_prefix}{name} as: {self.aliases[name]}"
        )
    ...
    def alias_replace(self, the_match):
        ...
        if the_match.group(1) and the_match.group(1) in self.aliases:
            replacement = self._path_delimiter.join(
                f'"{elem}"'
                for elem in self.aliases[the_match.group(1)][TextIndex._alias_path]
            )
        return replacement
        ...
        if len(self.entries) > 0:
            if self.depth > 1:
                if self.should_run_in:
                    ...
                    self.inform(
                        f"Deep index (>2 levels). Consider reducing depth, or enable run-in (see docs).",
                        severity="warning",
                    )
    ...
    def entry_html(self, entry):
        ...
        if (not entry.has_children() or run_in_children) and entry.has_also_refs():
            ...
            if alsorefs_html != None:
                if running_in:
                    refs_html += (
                        f" (<em>{self.see_also_label.lower()}</em> {alsorefs_html})"
                    )
                else:
    ...
    group_headings_enabled = property(
        get_group_headings_enabled, set_group_headings_enabled
    )
...
class TextIndexEntry:
    ...
    def add_reference(
        self, start_id, suffix=None, locator_emphasis=False, section=None
    ):
    ...
    def render_references(self):
        ...
        if len(self.references) > 0:
            ...
            for i in range(len(the_refs)):
                ...
                if (
                    self.textindex.section_mode and TextIndexEntry.section_end in ref
                ) or (not self.textindex.section_mode and TextIndexEntry.end_id in ref):
